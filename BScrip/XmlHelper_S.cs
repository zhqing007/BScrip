...
namespace BScrip
{
    ...
    public class XMLHelper_S
    {
        ...
        {
            return CreateXmlDocument(xmlFileName, rootNodeName, "1.0", "utf-8", "yes");
        }
        ...
        public static XmlNode CreateOrGetXmlNodeByXPath(string xmlFileName, string xpath, string xmlNodeName, string keyValue)
        {
            XmlDocument xmlDoc = new XmlDocument();
            xmlDoc.Load(xmlFileName); //加载XML文档
            XmlNode xmlNode = xmlDoc.SelectSingleNode(xpath);
            foreach (XmlNode node in xmlNode.ChildNodes){
                if (node.Name.ToLower() == xmlNodeName.ToLower() && node.Attributes["key"].Value.ToLower() == keyValue.ToLower())
                {
                    return node;
                }
            }
